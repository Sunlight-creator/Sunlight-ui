...
interface LineChartProps {
  // 数据配置
  data: SingleLineData | LineDataItem[]  // 支持单系列和多系列数据
  xAxisData: string[]                   // X轴数据
  height?: string                       // 图表高度，默认350px
  
  // 样式配置
  colors?: string[]                     // 颜色配置数组
  lineWidth?: number                    // 线宽，默认2.5
  showAreaColor?: boolean               // 是否显示区域填充，默认false
  smooth?: boolean                      // 是否平滑，默认true
  symbol?: string                       // 数据点类型，默认'none'
  symbolSize?: number                   // 数据点大小，默认6
...
const isEmpty = computed(() => {
  ...
  if (isMultipleSeries.value) {
    ...
    const singleData = props.data as SingleLineData
...
// 创建区域填充样式
const createAreaStyle = (color: string) => {
  ...
  if (isMultipleSeries.value) {
    ...
    return multiData.map((item, index) => {
      ...
      return {
        ...
        areaStyle: item.areaStyle || props.showAreaColor ? {
          color: createAreaStyle(color)
        } : undefined
      }
    })
  } 
...
const renderChart = () => {
  ...
  const option: echarts.EChartsOption = {
    ...
    legend: props.showLegend && isMultipleSeries.value ? {
      data: (props.data as LineDataItem[]).map(item => item.name),
