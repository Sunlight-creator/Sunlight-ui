...
interface ScatterDataItem {
  value: [number, number] // [x, y] 坐标值
  name?: string // 数据点名称
}

// 定义单系列数据类型
type SingleScatterData = ScatterDataItem[]

// 定义多系列数据项类型
interface ScatterSeriesItem {
  name: string // 系列名称
  data: ScatterDataItem[] // 系列数据
  symbolSize?: number // 系列符号大小
  color?: string // 系列颜色
}
// 定义组件属性接口
interface ScatterChartProps {
  // 数据配置
  data: SingleScatterData | ScatterSeriesItem[] // 支持单系列和多系列数据
  height?: string // 图表高度，默认350px

  // 样式配置
  colors?: string[] // 颜色配置数组，默认['#4C87F3', '#8BD8FC']
  symbolSize?: number // 符号大小，默认14
...
const isEmpty = computed(() => {
  ...
  if (isMultipleSeries.value) {
    const multiData = props.data as ScatterSeriesItem[]
    return multiData.every(item => !item.data || item.data.every(val => val.value.every(num => num === 0)))
  } else {
    const singleData = props.data as SingleScatterData
    return singleData.every(val => val.value.every(num => num === 0))
  }
})
  ...
  const option: echarts.EChartsOption = {
    ...
    legend:
      props.showLegend && isMultipleSeries.value
        ? {
            data: (props.data as ScatterSeriesItem[]).map(item => item.name),
