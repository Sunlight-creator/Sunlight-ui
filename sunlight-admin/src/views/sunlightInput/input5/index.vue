  ...
  <el-card class="box-card" shadow="hover">
    ...
    <template #header>
      <div class="card-header">
        <span class="title">SunlightInput 监听输入事件(input, change)</span>
      </div>
    </template>
    ...
    <div class="display-wrapper">
      <span class="label">当前输入内容：</span>
      <p class="text-content" v-if="value">{{ value }}</p>
      <p class="text-placeholder" v-else>暂无输入内容</p>
    </div>

    <!-- 当前事件展示 -->
    <div class="display-wrapper">
      <span class="label">当前触发事件：</span>
      <p class="text-content" v-if="currentEvent">{{ currentEvent }}</p>
      <p class="text-placeholder" v-else>暂无事件触发</p>
    </div>
...
function onInput(val) {
  ...
  currentEvent.value = `input 事件，值：${val}`
}

// 监听 change 事件
function onChange(val) {
  console.log('change:', val)
  currentEvent.value = `change 事件，值：${val}`
}
