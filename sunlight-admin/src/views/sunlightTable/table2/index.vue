  ...
  <div>
    <el-card>
      <SunlightTable
        ...
        v-model:radio-selected-key="selectedRadioKey"
...
function handleCustomButtonClick(key) {
  ...
  if (key === 'edit') {
    if (selectedRadioKey.value === undefined) {
      ElMessage.warning('请先选择一行数据')
      return
    }
    const selectedRow = tableData.value.find(row => row.id === selectedRadioKey.value)
...
const selectedRadioKey = ref(undefined)
